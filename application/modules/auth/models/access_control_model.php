...
class access_control_model extends Base_Model
{
	...
	function buildActionSelector()
	{
		...
		foreach($query->result() as $action)
		{
			...
			$value .= form_checkbox($checkbox,$action->name,$this->form_validation->set_checkbox($checkbox,$action->name));
			$value .= $action->name . "<br>\n";

			$value .= '<div id="'.$radio.'" class="action_item">';
			$value .= form_radio($radio,'Y',set_radio($radio,'Y',$selected = ($this->form_validation->$radio == 'Y') ? TRUE : FALSE)) . $this->lang->line('access_allow');
			$value .= form_radio($radio,'N',set_radio($radio,'N',$selected = ($this->form_validation->$radio == 'N') ? TRUE : FALSE)) . $this->lang->line('access_deny') . '</div>';
		}
		return $value;
	}
	...
	function _buildSelector($disabled,$type)
	{
		...
		while($obj->getTreeNext($tree))
		{
			...
			$lvl = $newLvl;
                        $selected = FALSE;
                        if(isset ($this->form_validation->$type)){
                            $selected = ($this->form_validation->$type == $tree['row']['name']) ? TRUE : FALSE;
                        }
			$set = $this->form_validation->set_radio($type,$tree['row']['name'],$selected);
