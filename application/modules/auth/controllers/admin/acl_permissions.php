...
class Acl_permissions extends Admin_Controller
{
	...
	function form($id = NULL)
	{
		...
		{
			$this->form_validation->set_default_value('allow_'.$action->id,'N');
		}
		...
		if( is_null($id))
		{
			...
			// Set form defaults
			$this->form_validation->set_default_value('allow','N');
			$this->form_validation->set_default_value('id','');
		}
		else
		{
			// MODIFY PERMISSION
			...
			// Fetch form data
			$this->form_validation->set_default_value('id',$id);
			$result = $this->access_control_model->getPermissions(NULL,array('acl.id'=>$id));
			$row = $result[$id];

			$this->form_validation->set_default_value('aro',$row['aro']);
			$this->form_validation->set_default_value('aco',$row['aco']);
			$this->form_validation->set_default_value('allow',($row['allow']?'Y':'N'));

			if( isset($row['actions']))
			{
				foreach($row['actions'] as $action)
				{
					$this->form_validation->set_default_value('action_'.$action['id'],$action['axo']);
					$this->form_validation->set_default_value('allow_'.$action['id'],($action['allow']?'Y':'N'));
				}
			}
		}
