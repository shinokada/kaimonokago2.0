	...
	function form($id = NULL)
	{
		...
		$this->form_validation->set_fields($fields);
                ...
                $config[] = array(
                                    ...
                                    'rules'=>"trim|required|max_length[254]"
                                    );
                $config[] = array(
                                    'field'=>'parent',
                                    'label'=>$this->lang->line('access_parent_name'),
                                    'rules'=>"required"
                                    );
                $config[] = array(
                                    'field'=>'disabled',
                                    'label'=>$this->lang->line('access_disabled'),
                                    'rules'=>"required"
                                    );
		...
		if( ! is_null($id) AND ! $this->input->post('submit'))
		{
			...
			$this->form_validation->set_default_value('disabled',$row->disabled);
			$this->form_validation->set_default_value('name',$node['name']);
			$this->form_validation->set_default_value('parent',$parent['name']);
		}
		elseif( is_null($id) AND ! $this->input->post('submit'))
		{
			// Create form, first load
			$this->form_validation->set_default_value('disabled','0');
		}
